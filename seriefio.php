...
// 데이터베이스에서 항목 가져오기
$result = db_select("SELECT * FROM contents WHERE content_code LIKE '%-1410%' $orderPhrase LIMIT $start, $itemsPerPage");

// 페이지 링크를 만들기 위한 전체 항목 개수 계산
$totalItems = db_select("SELECT COUNT(*) as count FROM contents WHERE content_code LIKE '%-1410'")[0]['count'];
...
<body>
    ...
    <div class="picked_category">
        <span class="category_index">ACF 피오렌티나 상품</span>
        <div><br></div>
        <div><br></div>
        <div class="soccer">
            <ul>
    ...
    <main class="main_wrapper">
        <div class="recommend_main_wrapper">
            ...
            <script>
                ...
                SortNew.addEventListener('click', function() {
                    location.href = 'seriefio.php?sort=SUBSTRING(content_code,1,6)&dir=desc&btn=1'
                });
                //높은가격순으로 정렬
                SortHighPrice.addEventListener('click', function() {
                    location.href = 'seriefio.php?sort=content_price&dir=desc&btn=3'
                });
                //낮은가격순으로 정렬
                SortLowPrice.addEventListener('click', function() {
                    location.href = 'seriefio.php?sort=content_price&dir=asc&btn=4'
                });
                //가나다순으로 정렬
                SortKor.addEventListener('click', function() {
                    location.href = 'seriefio.php?sort=content_name&dir=asc&btn=2'
                });
            </script>
            <div><br></div>
            <div><br></div>
        ...
        <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
            <a href="seriefio.php?sort=<?php echo $sort; ?>&dir=<?php echo $dir; ?>&btn=<?php echo $btn; ?>&page=<?php echo $i; ?>"
